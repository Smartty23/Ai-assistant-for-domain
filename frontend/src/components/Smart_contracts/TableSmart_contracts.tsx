...
import { loadColumns } from './configureSmart_contractsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import ListSmart_contracts from './ListSmart_contracts';

const perPage = 10;

const TableSampleSmart_contracts = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    loading,
    count,
    notify: smart_contractsNotify,
    refetch,
  } = useAppSelector((state) => state.smart_contracts);
  ...
  useEffect(() => {
    if (smart_contractsNotify.showNotification) {
      notify(
        smart_contractsNotify.typeNotification,
        smart_contractsNotify.textNotification,
      );
    }
  }, [smart_contractsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/smart_contracts/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/smart_contracts/smart_contracts-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {smart_contracts && Array.isArray(smart_contracts) && !showGrid && (
        <ListSmart_contracts
...
export default TableSampleSmart_contracts;
