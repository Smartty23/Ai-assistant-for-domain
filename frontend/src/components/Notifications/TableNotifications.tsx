...
const TableSampleNotifications = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  useEffect(() => {
    if (notificationsNotify.showNotification) {
      notify(
        notificationsNotify.typeNotification,
        notificationsNotify.textNotification,
      );
    }
  ...
  const handleViewAction = (id: string) => {
    router.push(`/notifications/notifications-view/?id=${id}`);
  };
