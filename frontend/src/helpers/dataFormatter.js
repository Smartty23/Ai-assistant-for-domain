...
export default {
  ...
  smart_contractsManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.name);
  },
  smart_contractsOneListFormatter(val) {
    if (!val) return '';
    return val.name;
  },
  smart_contractsManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.name };
    });
  },
  smart_contractsOneListFormatterEdit(val) {
    if (!val) return '';
